...
-(void)doOldStyleForConnect{
    ...
    if (server_socket == -1) {
        ...
        server_addr.sin_port = htons(12103);
        server_addr.sin_addr.s_addr = inet_addr("192.168.1.102");// inet_addr("127.0.0.1");
...
- (void)acceptFromServer{
    ...
    while (1) {
        //接受服务器传来的数据
        char buf[1024];
        long iReturn = recv(self.server_socket, buf, 10240, 0);
        if (iReturn>0) {
            NSLog(@"irrent:%ld",iReturn);
